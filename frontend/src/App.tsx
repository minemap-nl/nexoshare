...
import { 
  ...
  MessageSquare, Type, Calendar, Sparkles
} from 'lucide-react';
...
const Footer = ({ transparent = false }: { transparent?: boolean }) => (
    <footer className={`w-full py-6 text-center text-neutral-500 text-sm mt-auto ${transparent ? '' : 'border-t border-neutral-800 bg-black backdrop-blur-sm'}`}>
        <p className="font-medium tracking-wide">Created by Minemap x Famretera</p>
    </footer>
);
    ...
    return (
        <UIContext.Provider value={{ notify, confirm }}>
            ...
            <div className="fixed top-5 right-2 md:right-5 z-[10001] flex flex-col gap-3 pointer-events-none max-w-[calc(100vw-1rem)] md:max-w-none">
                {toasts.map(t => (
                    <div key={t.id} className={`anim-toast pointer-events-auto min-w-[280px] md:min-w-[300px] p-3 md:p-4 rounded-xl shadow-2xl border flex items-center gap-2 md:gap-3 text-white backdrop-blur-md ${
                        t.type === 'success' ? 'bg-green-600/90 border-green-500' : 
...
const UploadView = () => {
  ...
  const handleDrop = (e: any) => { 
      ...
      if (e.target.files || e.dataTransfer.files) {
          setFiles([...files, ...Array.from((e.target.files || e.dataTransfer.files) as FileList)]);
          // Reset de input value zodat hetzelfde bestand opnieuw gekozen kan worden na verwijdering
  ...
  return (
    <div className="relative anim-fade">
      <div className="bg-neutral-900 border-2 border-dashed border-neutral-800 rounded-2xl p-6 md:p-10 flex flex-col items-center justify-center min-h-[250px] md:min-h-[300px] cursor-pointer hover:border-purple-500 hover:bg-neutral-900/80 transition-all duration-300 group" onClick={() => fileInputRef.current?.click()} onDragOver={e => e.preventDefault()} onDrop={handleDrop}>
        <input ref={fileInputRef} type="file" multiple className="hidden" onChange={handleDrop} />
      ...
      {files.length > 0 && (
          <div className="mt-8 anim-slide bg-neutral-900 rounded-2xl border border-neutral-800 overflow-hidden shadow-xl">
                <div className="max-h-[300px] overflow-y-auto">
                    {files.map((f, i) => (
                        <div key={i} className="flex justify-between items-center p-3 md:p-4 border-b border-neutral-800 last:border-0 hover:bg-neutral-800/50 transition gap-2">
                            <div className="flex items-center gap-2 md:gap-4 overflow-hidden flex-1 min-w-0">
                                <div className="bg-black p-2 rounded-lg flex-shrink-0"><div className="uppercase text-xs font-bold text-purple-400">{f.name.split('.').pop()}</div></div>
                                <div className="min-w-0 flex-1">
                                    <p className="text-white font-medium truncate text-sm md:text-base">{f.name}</p>
                                    <p className="text-neutral-500 text-xs">{formatBytes(f.size)}</p>
                                </div>
                            </div>
                            <button onClick={(e) => {e.stopPropagation(); setFiles(files.filter((_, idx) => idx !== i))}} className="text-neutral-500 hover:text-red-400 p-2 transition flex-shrink-0"><X className="w-4 h-4 md:w-5 md:h-5"/></button>
                        </div>
                    ))}
                </div>
...
const ReverseView = () => {
    ...
    const openFiles = async (id: string) => { const res = await fetch(`${API_URL}/reverse/${id}/files`, { credentials: 'include' }); setUploadedFiles(await res.json()); setViewFiles(id); };
        ...
        <div className="bg-neutral-900 rounded-xl border border-neutral-800 p-6 anim-scale">
            ...
            {uploadedFiles.length === 0 ? <p className="text-neutral-500">Nothing uploaded yet.</p> : (
                <div className="space-y-2">
                    {uploadedFiles.map(f => (<div key={f.id} className="flex justify-between items-center p-3 bg-black rounded border border-neutral-800 text-neutral-300"><span>{f.original_name} ({formatBytes(f.size)})</span><a href={`${API_URL}/reverse/files/${f.id}/download`} className="text-purple-400 hover:text-white transition p-2 rounded hover:bg-neutral-800"><Download className="w-4 h-4"/></a></div>))}
                    <div className="mt-4 pt-4 border-t border-neutral-700"><a href={`${API_URL}/reverse/${viewFiles}/download`} className="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded font-bold inline-block transition btn-press shadow-lg shadow-green-900/20">Download everything (.zip)</a></div>
                </div>
            )}
        </div>
    );
            ...
            <div className="p-4 md:p-8">
                ...
                {activeTab === 'system' && (
                    <div className="space-y-6 anim-fade">
                        ...
                        <h3 className="text-white font-bold text-xl mt-8 mb-6 flex gap-2"><Shield className="w-6 h-6 text-purple-500"/> Server Security</h3>
                        <div className="space-y-4 bg-red-500/5 p-4 rounded-xl border border-red-500/20 mb-6">
                            <div className="flex items-start gap-3">
                                <Checkbox 
                                    checked={config.clamavMustScan || false} 
                                    onChange={e => setConfig({...config, clamavMustScan: e.target.checked})} 
                                    className="mt-1" // Klein beetje zakken om uit te lijnen met de tekst
                                />
                                <div>
    ...
    return (
        <div className="min-h-screen bg-black flex items-center justify-center p-4 relative overflow-hidden flex-col">
            ...
            <form onSubmit={twoFactorRequired ? handleVerify2FA : handleSubmit} className="bg-neutral-900 p-6 md:p-10 rounded-2xl w-full max-w-md border border-neutral-800 shadow-2xl relative z-10 anim-scale mb-8">
                ...
                <h1 className="text-3xl font-bold text-white mb-8 text-center">{config.appName || 'Login'}</h1>
        ...
        <div className="min-h-screen bg-black flex items-center justify-start pt-24 md:pt-32 p-4 flex-col">
            ...
            <div className="bg-neutral-900 p-6 md:p-8 rounded-2xl border border-neutral-800 max-w-lg w-full anim-slide shadow-2xl mb-8">
                ...
                {!unlocked ? (
                    ...
                    <div className="space-y-6 anim-fade">
                        <div className="border-2 border-dashed border-neutral-700 rounded-xl p-8 text-center bg-black/50 relative hover:border-purple-500 transition cursor-pointer" onClick={() => fileInputRef.current?.click()}>
                            {/* Voeg nieuwe files toe aan de bestaande lijst (spread operator) i.p.v. vervangen */}
                            <input ref={fileInputRef} type="file" multiple className="hidden" onChange={e => setFiles([...files, ...Array.from(e.target.files || [])])} />
                            <p className="text-neutral-300">Click or drag files</p>
                        </div>
                        {files.length > 0 && <div className="bg-black rounded-lg p-4 max-h-40 overflow-y-auto border border-neutral-700">{files.map((f, i) => <div key={i} className="text-neutral-300 text-sm border-b border-neutral-800 py-2 last:border-0">{f.name}</div>)}</div>}
...
const DownloadPage = () => {
    ...
    const [error, setError] = useState<string | null>(null); // Nieuwe state voor errors
    const [password, setPassword] = useState('');
    const { notify } = useUI();
    ...
    const verify = async () => { const res = await fetch(`${API_URL}/shares/${id}/verify`, { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify({password})}); const json = await res.json(); if(json.valid) setData({...data, files: json.files, protected: false}); else notify('Wrong password', "error"); };
            ...
            <div className="bg-neutral-900 rounded-2xl p-6 md:p-8 max-w-lg w-full border border-neutral-800 shadow-2xl anim-scale mb-8">
                ...
                {data.protected ? (
                    ...
                    <div className="anim-fade">
                        ...
                        <div className="space-y-2 mb-6 max-h-64 overflow-y-auto bg-black/50 rounded-lg p-2 border border-neutral-800">
                            {data.files.map((f: any, i: number) => (
                                <div key={i} className="flex justify-between items-center bg-black p-3 rounded-lg border border-neutral-800 hover:border-neutral-600 transition">
                                    <div className="min-w-0 pr-4">
                                        <p className="text-neutral-200 text-sm truncate">{f.original_name}</p>
                                        <p className="text-neutral-500 text-xs">{formatBytes(f.size)}</p>
                                    </div>
